...
export const onDragNode: IStateCallback<IOnDragNode> = ({
  config,
  event,
  data,
  id
...
export const onDragCanvas: IStateCallback<IOnDragCanvas> = ({
  config,
  event,
  data
}) => (chart: IChart): IChart => {
  // console.log("onDragCanvas action.ts");
...
export const onDragCanvasStop: IStateCallback<IOnDragCanvasStop> = () => (
  ...
  for (let key in chart.nodes) {
    let node = chart.nodes[key];
    node.position = {
      x: chart.offset.x + node.position.x,
      y: chart.offset.y + node.position.y
    };
  }
...
export const onLinkCancel: IStateCallback<IOnLinkCancel> = ({ linkId }) => (
  chart: IChart
) => {
  delete chart.links[linkId];
  return chart;
...
export const onLinkClick: IStateCallback<IOnLinkMouseLeave> = ({ linkId }) => (
  chart: IChart
) => {
...
export const onCanvasClick: IStateCallback<IOnCanvasClick> = () => (
  ...
  console.log("onCanvasClick action.ts");
...
export const onDeleteKey: IStateCallback<IOnDeleteKey> = () => (
  chart: IChart
) => {
...
export const onNodeClick: IStateCallback<IOnNodeClick> = ({ nodeId }) => (
  chart: IChart
) => {
...
export const onNodeSizeChange: IStateCallback<IOnNodeSizeChange> = ({
  nodeId,
  size
}) => (chart: IChart) => {
  chart.nodes[nodeId] = {
